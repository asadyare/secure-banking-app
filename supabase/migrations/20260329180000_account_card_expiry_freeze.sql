-- Card-style expiry (MM/YY) and optional freeze (blocks outgoing transfers).

ALTER TABLE public.accounts
  ADD COLUMN IF NOT EXISTS card_expiry text NOT NULL DEFAULT '05/30';

ALTER TABLE public.accounts
  ADD COLUMN IF NOT EXISTS is_frozen boolean NOT NULL DEFAULT false;

-- Deterministic varied expiry per row (looks like real cards)
UPDATE public.accounts
SET card_expiry =
  lpad(((abs(hashtext(id::text)) % 12) + 1)::text, 2, '0')
  || '/'
  || lpad(((abs(hashtext(id::text) / 7) % 6) + 25)::text, 2, '0')
WHERE card_expiry = '05/30';

REVOKE UPDATE ON public.accounts FROM authenticated;
GRANT UPDATE (account_name, account_type, is_active, currency, is_frozen) ON public.accounts TO authenticated;
...
BEGIN
  ...
  SELECT user_id, balance, is_frozen INTO v_from_owner, v_from_balance, v_from_frozen
  ...
  IF COALESCE(v_from_frozen, false) THEN
    RAISE EXCEPTION 'This account is frozen. Unfreeze it to send money.';
  END IF;
...
-- New signups: assign a card-style expiry (same formula as backfill)
...
BEGIN
  ...
  INSERT INTO public.accounts (user_id, account_number, account_type, account_name, balance, card_expiry)
  VALUES (
    NEW.id,
    ...
    'Primary Checking',
    9999999999999.99,
    lpad(((abs(hashtext(NEW.id::text)) % 12) + 1)::text, 2, '0')
      || '/'
      || lpad(((abs(hashtext(NEW.id::text) / 7) % 6) + 25)::text, 2, '0')
  );
